...
export default function ShopIntegrations() {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get(`/api/admin/integrations?shopId=${shopId}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        const res = await axios.put(`/api/admin/integrations/${editing.id}`, { ...form, shopId });
        setIntegrations(integrations.map(i => i.id === editing.id ? res.data : i));
        setEditing(null);
      } else {
        const res = await axios.post('/api/admin/integrations', { ...form, shopId });
  ...
  return (
    <div className="container mx-auto max-w-2xl py-10">
      <h1 className="text-2xl font-thunder mb-6">Intégrations de la boutique</h1>
